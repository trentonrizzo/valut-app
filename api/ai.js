 ...
 * POST body.action = 'chat' | 'analyze'
 * Without OPENAI_API_KEY / VAULT_AI_API_KEY returns 503 (client falls back to metadata tools).
 */

const ANALYSIS_VERSION = '1'

async function handleChat(req, res, key, body) {
...
async function handleAnalyze(req, res, key, body) {
  const images = Array.isArray(body?.images) ? body.images.slice(0, 4) : []
  if (!images.length) return sendJson(res, 400, { error: 'images required (data URLs or base64 jpeg)' })

  const focus = String(
    body?.focus ||
      'general visual description, approximate people count, hair color if visible, useful searchable descriptors',
  ).slice(0, 500)
