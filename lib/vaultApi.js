...
export async function uploadToVault(file, userId, album, accessToken) {
  ...
  console.info('[UPLOAD] client request', {
    album,
    userId,
    fileName: file && typeof file.name === 'string' ? file.name : 'blob',
    fileType: file.type,
    fileSize: file.size,
  })
  ...
  console.info('[UPLOAD] client response', data)

  if (!data?.ok || typeof data?.url !== 'string' || !data.url) {
    throw new Error('Upload response missing required fields (ok/url)')
  }
